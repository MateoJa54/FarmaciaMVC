    ...
    public class LaboratorioDAL : CadenaDAL
    {
        ...
        public LaboratorioCLS RecuperarLaboratorio(int idLaboratorio)
        {
            ...
            using (SqlConnection cn = new SqlConnection(this.cadena))
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("select IIDLABORATORIO, NOMBRE, DIRECCION, PERSONACONTACTO, NUMEROCONTACTO \r\n" +
                        " from Laboratorio\r\nwhere BHABILITADO = 1 and IIDLABORATORIO = @iidlaboratorio ", cn))
                    {
                        cmd.CommandType = System.Data.CommandType.Text;
